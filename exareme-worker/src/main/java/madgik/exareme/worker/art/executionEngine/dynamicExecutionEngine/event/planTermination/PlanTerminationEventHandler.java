...
public class PlanTerminationEventHandler implements ExecEngineEventHandler<PlanTerminationEvent> {
	...
	@Override
	public void preProcess(PlanTerminationEvent event, PlanEventSchedulerState state) throws RemoteException {
		if (state.isTerminated() == false) {
			for (ContainerProxy proxy : state.getContainerProxies()) {
				try {
					...
					// throw new ServerException("Cannot close all sessions",
					// e);
				}
			}
	...
	@Override
	public void handle(PlanTerminationEvent event, EventProcessor proc) throws RemoteException {
	}

	@Override
	public void postProcess(PlanTerminationEvent event, PlanEventSchedulerState state) throws RemoteException {
