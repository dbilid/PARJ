...
public class RmiAdpDBSelectOptimizer {
	...
	private static final boolean filterColumns = AdpDBProperties.getAdpDBProps()
			.getBoolean("db.execution.filterUnusedColumns");
	...
	public AdpDBQueryExecutionPlan optimize(InputData input, AdpDBClientProperties props) throws RemoteException {
		...
		FastSubgraphFilter subgraphFilter = new FastSubgraphFilter(sketchState.graph.getNumOfOperators());
		...
		RmiAdpDBOptimizer.annotateWithHistoricalData(sketchState.graph, sketchInput.queryData, sketchState.dbOps,
				props);
		...
		if (props.isTreeEnabled()) {
			...
			// sketchState.graph, subgraphFilter, sketchState.containers,
			// sketchState.contFilter);
		} else {
			sketchState.space = RmiAdpDBSelectScheduler.schedule(sketchState.graph, subgraphFilter,
					sketchState.containers, sketchState.contFilter);
		}
		log.debug("Creating final graph from sketch ...");
		StateData state = new StateData();
		createFinalGraphFromSketch(sketchInput, input, sketchState, state, props);
		String skyChoice = AdpDBProperties.getAdpDBProps().getString("db.optimizer.scheduler.plan.choice");
		...
		ConsoleUtils.printStatisticsToLog(skyChoice + " " + state.result.getStatistics().getContainersUsed(),
				state.graph, RmiAdpDBSelectScheduler.runTimeParams, state.result.getStatistics(), log, Level.DEBUG);
	...
	private void createSketchInput(InputData input, InputData sketchInput, AdpDBClientProperties props)
	...
	private void createProxies(InputData input, StateData state, AdpDBClientProperties props) throws RemoteException {
	...
	private void addVirtualContainers(InputData input, StateData state) throws RemoteException {
		...
		if (AdpProperties.getEnvProps().getString("run_level").equals("develop")) {
			...
			HashMap<String, ContainerProxy> virtualContainers = new HashMap<String, ContainerProxy>();
			for (PhysicalTable pt : input.schema.getPhysicalTables()) {
				...
				for (int p = 0; p < pt.getNumberOfPartitions(); ++p) {
					...
					for (String location : part.getLocations()) {
						if (virtualContainers.containsKey(location)
								|| state.proxies[0].getEntityName().getName().startsWith(location)) {
							continue;
						}
						for (int cpu = 0; cpu < VIRTUAL_CPUS_PER_CONTAINER; ++cpu) {
							...
							ContainerProxy vProxy = new RmiContainerProxy(NetUtil.getIPv4(),
									location + "_container_" + cpu, ArtRegistryLocator.getLocalRmiRegistryEntityName());
	...
	private void createContainerFilter(InputData input, StateData state, AdpDBClientProperties props) {
		if (input.maxNumCont < 0) {
			log.debug("No container filter used!");
			state.contFilter = NoContainerFilter.getInstance();
		} else {
			log.debug("Creating container filter " + "(Greedy algorithm that select those with most tables) ...");
			ArrayList<Pair<Integer, Integer>> contTablesCntPairs = new ArrayList<Pair<Integer, Integer>>();
			for (int i = 0; i < state.proxies.length; ++i) {
			...
			for (int i = 0; i < input.maxNumCont; ++i) {
				log.debug("Selected container " + contTablesCntPairs.get(i).a + " with " + contTablesCntPairs.get(i).b
						+ " table partitions.");
	...
	private SchedulingResult getSchedule(String scheduleChoice, SolutionSpace space) throws RemoteException {
	...
	private void bindTablePartition(PhysicalTable pTable, int p, ConcreteOperator reader, InputData input,
		...
		for (int i = contFilter.nextSetBit(0), l = 0; i >= 0; i = contFilter.nextSetBit(i + 1), l++) {
			locations[l] = i;
		}
		...
		for (int loc : locations) {
			...
			if (state.containerTablePartCounts[minContainer] > state.containerTablePartCounts[loc]) {
				minContainer = loc;
			}
		}
	...
	private void prepareInput(Select query, ProcessQueryState queryState, InputData input, StateData state, AdpDBClientProperties props)
		...
		for (TableView inTableView : query.getInputTables()) {
			...
			if (pTable == null) {
				...
				Check.True(pTable.getTable().getLevel() >= 0, "Intermediate table level not set: " + pTable.getName());
			} else {
			...
			if (inTableView.getPattern() != inputPattern) {
				throw new SemanticException("All the patterns of the input must be the same: "
						+ inTableView.getPattern() + " != " + inputPattern);
			}
		}
	}
	...
	private void applyInputPattern(Select query, ProcessQueryState queryState, InputData input, StateData state , AdpDBClientProperties props)
			...
			for (TableView inTableView : query.getInputTables()) {
				...
				if (numOfPartitions == 1) {
					...
					if (numOfPartitions != producers.length) {
						throw new SemanticException("Direct format exception: " + query.toString());
					}
				}
			}
		...
		case tree: {
			...
			Check.True(query.getInputTables().size() == 1, "Only one input table can be used with tree reduction");
			...
			for (int oPart = 0; oPart < outputPartitions; ++oPart) {
				if (runOnParts.size() > 0) {
					...
					if (found < 0) {
						...
						log.debug("Part of table '" + outTableView.getName() + "' not used: " + oPart);
						continue;
					}
				}
				...
				if (inputPattern == DataPattern.external) {
					...
					state.tableBindings.add(new Pair<ConcreteOperator, Integer>(runQuery, localContainer));
				...
				if (inputPattern == DataPattern.remote) {
					...
					Check.NotNull(containerProxy, "proxy not found: " + containerProxy.getEntityName().getIP());
					log.debug("proxy container is: " + containerProxy.getEntityName().getName());
					state.tableBindings.add(new Pair<ConcreteOperator, Integer>(runQuery, containerCounter));
					state.containerTablePartCounts[containerCounter]++;
					containerCounter++;
				}
				if (inputPattern == DataPattern.virtual) {
					log.debug("Bind virtual operators to all machines.");
					state.tableBindings.add(new Pair<ConcreteOperator, Integer>(runQuery, oPart));
					state.containerTablePartCounts[oPart]++;
				}
				// Filter only the specified partitions
				for (int i = 0; i < inputs.size(); ++i) {
					ConcreteOperator fromOp = inputs.get(i)[(inputs.get(i).length == 1) ? 0 : oPart];
					if (fromOp == null) {
						continue;
					}
					Link link = ConcreteGraphFactory.createLink(oPart + "." + i, fromOp, runQuery);
					...
					if (dbOp.addToInputsAllOutputsOf(fromDbOp) != 1) {
						throw new SemanticException("The input of a db operator from another operator must be one!");
					}
				}
		...
		case tree: {
			Check.True(runOnParts.isEmpty(), "Tree reduction cannot be combined with partition selection!");
	...
	private void applyOutputPattern(Select query, ProcessQueryState queryState, InputData input, StateData state, AdpDBClientProperties props)
			...
			if (numOfOutPartitions != 1) {
				if (outTableView.getPatternColumnNames().isEmpty()) {
					throw new SemanticException(
							"Output pattern 'many' with more than one outputs, " + "must specify at least one column.");
				}
			}
			break;
		}
		...
		// The level of the table is one more than the highest level of the
		// input tables.
		level++;
		...
		for (int oPart = 0; oPart < numOfOutPartitions; ++oPart) {
			...
			// If the output is the same, add only one post output to each
			// output.
	...
	private void bindLocationOFResultTables(InputData input, StateData state) {
		// Bind the output table producers to the location of the result. For
		// now, use round robin!
		if (state.resultTableName != null) {
	...
	private void createFinalGraphFromSketch(InputData sketchInput, InputData input, StateData sketchState,
		...
		log.debug("Bind " + state.tableBindings.size() + " operators to the location of tables ... ");
		...
		log.debug("Annotating graph with historical data ...");
		RmiAdpDBOptimizer.annotateWithHistoricalData(state.graph, input.queryData, state.dbOps, props);
		log.debug("Adding local file data ...");
