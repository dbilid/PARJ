...
public class FederatedAnalyzer implements Analyzer {
	...
	public FederatedAnalyzer(String ipAddress, int portNo, String user, String pass, String db, String vendorName)
			throws Exception {
	...
	private void createSample() throws Exception {
		...
		for (String tableName : this.statCols.keySet()) {
			...
			switch (this.vendor) {
			case Oracle:
				sampleQuery = "select * from (oracle jdbc:oracle:thin:@" + this.ip + ":" + this.port + ":" + this.dbName
						+ " u:" + this.username + " p:" + this.password + "  select * from ( select * from   "
						+ tableName + " order by dbms_random.value() ) where ROWNUM <= 1000  UNION ALL " + mm.toString()
						+ " );";
				break;
			case Mysql:
				// sampleQuery =
	...
	public void createConnection(String vendorName) throws Exception {
		if (vendorName.equals("Oracle")) {
			...
			con = DriverManager.getConnection("jdbc:oracle:thin:@" + this.ip + ":" + this.port + ":" + this.dbName,
					this.username, this.password);
		} else if (vendorName.equals("Mysql")) {
			this.vendor = Vendor.Mysql;
			Class.forName("com.mysql.jdbc.Driver");
			con = DriverManager.getConnection("jdbc:mysql://" + this.ip + ":" + this.port + "/" + this.dbName,
					this.username, this.password);
