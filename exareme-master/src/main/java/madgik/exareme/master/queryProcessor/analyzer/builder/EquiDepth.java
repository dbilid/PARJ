	...
	public Schema build(Map<String, Table> dbStats) {
		...
		for (Map.Entry<String, Table> t : dbStats.entrySet()) {
			...
			for (Map.Entry<String, Column> c : t.getValue().getColumnMap().entrySet()) {
				...
				TreeMap<Double, Integer> sdata = sortData(
						t.getValue().getColumnMap().get(c.getKey()).getDiffValFreqMap());
				...
				while (it.hasNext()) {
					...
					if (c.getKey().equals("l_partkey")) {
						System.out.println("==================> " + "curdepth: " + curDepth + " bucket_depth: "
				...
				for (Entry<Double, Bucket> e : bucketIndex.entrySet()) {
					fscale = ((e.getValue().estimateBucketNumberOfTuples() * (double) t.getValue().getNumberOfTuples())
							/ (double) totalSampledRows) / e.getValue().getDiffValues();
					e.getValue().setFrequency(fscale);
				}

				bucketIndex.put(Math.nextAfter(lastHistVal, Double.MAX_VALUE), Bucket.FINAL_HISTOGRAM_BUCKET);

				attrIndex.put(c.getKey(),
						new AttrInfo(c.getKey(), new Histogram(bucketIndex), c.getValue().getColumnLength()));

				System.out.println("ATTR_NAME: " + c.getKey() + " NUM_BUCKETS: "
						+ attrIndex.get(c.getKey()).getHistogram().getBucketIndex().size() + " HIST_TUPLES: "
						+ attrIndex.get(c.getKey()).getHistogram().numberOfTuples());
			}

			Set<String> hs = new HashSet<String>();
			hs.add(t.getValue().getPrimaryKey());

			relMap.put(t.getKey(), new RelInfo(t.getKey(), attrIndex, dbStats.get(t.getKey()).getNumberOfTuples(),
