...
public class Column {
	...
	public Column(String columnName, int columnType, int columnSize, int numberOfDiffValues, String minValue,
			String maxValue, Map<String, Integer> diffValFreqMap) {
		...
		if (StatUtils.isTextType(this.columnType)) {
			...
			for (Entry<String, Double> e : this.hashStringMap.entrySet()) {
				if (!tempDiffFreqMap.containsKey(e.getValue().toString())) {
					...
					tempDiffFreqMap.put(e.getValue().toString(), f + diffValFreqMap.get(e.getKey()));
				}
			}
	...
	public String toString() {
		return "Column{" + "columnName=" + columnName + ", columnType=" + columnType + ", columnSize=" + columnLength
				+ ", numberOfDiffValues=" + numberOfDiffValues + ", minValue=" + minValue + ", maxValue=" + maxValue
				+ ", diffValFreqMap=" + diffValFreqMap + '}';
	}
