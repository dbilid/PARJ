...
public class Memo {
	...
	public void put(Node e, SinglePlan resultPlan, Column c, double repCost, boolean b, PartitionCols l) {
		MemoKey k = new MemoKey(e, c);
		PartitionedMemoValue v = new PartitionedMemoValue(resultPlan, repCost);
		v.setMaterialized(b);
		v.setDlvdPart(l);
	...
	public void setPlanUsed(MemoKey e) {
		...
		for (int i = 0; i < p.noOfInputPlans(); i++) {
			...
			// if(sp.getNode().getDescendantBaseTables().size()==1 &&
			// !this.getMemoValue(sp).isFederated()){
			// continue;
			// }
			setPlanUsed(sp);
	...
	public void put(Node e, SinglePlan resultPlan, boolean materialized, boolean used, boolean federated) {
		MemoKey k = new MemoKey(e, null);
		CentralizedMemoValue v = new CentralizedMemoValue(resultPlan);
		v.setFederated(federated);
