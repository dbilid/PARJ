...
public class EquivalentColumnClasses {
	...
	public void addClassesFrom(EquivalentColumnClasses other) {
		 ...
		 * if(!this.classes.containsKey(pc)){ for(Column c:pc.getColumns()){
		 * PartitionCols pc2=this.getClassForColumn(c); if(!pc2.isEmpty()){
		 * 
		 * } } } else{ this.classes.put(pc, other.classes.get(pc)); } }
		 */
	}
