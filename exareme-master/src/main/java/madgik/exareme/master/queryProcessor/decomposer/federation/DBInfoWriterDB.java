...
public class DBInfoWriterDB {
		...
		try {
			...
			if (query.startsWith("addFederatedEndpoint")) {
				...
				if (params[2].equalsIgnoreCase("com.mysql.jdbc.Driver")) {
					...
					madisString = "mysql " + "h:" + host + " u:" + params[3] + " p:" + params[4] + " db:"
					...
					madisString = "postgres " + "h:" + host + " port:" + port + " u:" + params[3] + " p:" + params[4]
							+ " db:" + db;
				}
				statement.executeUpdate("DELETE FROM endpoint WHERE id = '" + params[0].toUpperCase() + "';");
				statement.executeUpdate("INSERT INTO endpoint VALUES ('" + params[0].toUpperCase() + "','" + params[1]
	...
	public static void writeAliases(NamesToAliases n2a, String directory) throws ClassNotFoundException {
		...
		try {
			...
			if (!tables.next()) {
				statement.executeUpdate("create table aliases (tablename string, aliases string)");
			}
			...
			for (String tablename : n2a.getTables()) {
				...
				statement.executeUpdate("INSERT INTO aliases VALUES ('" + tablename + "','" + aliasesForTable + "');");
