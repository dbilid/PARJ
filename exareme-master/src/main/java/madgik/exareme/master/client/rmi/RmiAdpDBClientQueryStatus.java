...
public class RmiAdpDBClientQueryStatus implements AdpDBClientQueryStatus {
	...
	public RmiAdpDBClientQueryStatus(AdpDBQueryID queryId, AdpDBClientProperties properties,
			AdpDBQueryExecutionPlan plan, AdpDBStatus status, AdpDBClient client) {
		this.properties = properties;
		this.plan = plan;
		this.status = status;
		this.resultTableName = null;
		...
		if (status.hasError() == false) {
			...
			for (ContainerSessionStatistics containerStat : status.getStatistics()
				...
				for (ConcreteOperatorStatistics operatorStatistics : containerStat.operators) {
					ExecuteQueryExitMessage exitMessage = (ExecuteQueryExitMessage) operatorStatistics.getExitMessage();
					// get
					if (exitMessage != null) {

						// if
						// (AdpDBProperties.getAdpDBProps().getString("db.cache").equals("true"))
						// {
						if (properties.isCachedEnable()) {
							if (exitMessage.type == AdpDBOperatorType.tableUnionReplicator) {
								...
								sqlQueries.put(exitMessage.outTableInfo.getTableName(),
										exitMessage.outTableInfo.getSqlQuery());
							}
						} else {
			...
			for (PhysicalTable resultTable : plan.getResultTables()) {
				...
				if (properties.isCachedEnable()) {
					...
					for (int part = 0; part < resultTable.getPartitions().size(); ++part) {
						...
						// resultTable.getPartitions().get(part).getpNum());
					...
					if (resultTable.getTable().hasSQLDefinition() == false) {
						if (resultTables.containsKey(resultTable.getName()) == false) {
							throw new SemanticException("Table definition not found: " + resultTable.getName());
						}
					...
					cache.unpinTable(resultTable.getName()); // paizei na mn
																// exei noima
																// edw mias kai
																// anaferete se
																// pernament
					...
					try {
						sqlQuery = SQLQueryParser.parse(
								resultTable.getTable().getSqlQuery().replaceAll("_", " ").replaceAll("\\ {2,}", "_"),
								new madgik.exareme.master.queryProcessor.decomposer.dag.NodeHashValues());

						// List<Table> usedTables = new
						// ArrayList<>(sqlQuery.getInputTables().size());
						for (madgik.exareme.master.queryProcessor.decomposer.query.Table usedTable : sqlQuery
								.getInputTables()) {
			...
			// (AdpDBProperties.getAdpDBProps().getString("db.cache").equals("true"))
			// {
			List<String>  evictedTables = new ArrayList<String>();
			if (properties.isCachedEnable()) {
				...
				cache = new Cache(properties,
						Integer.parseInt(AdpDBProperties.getAdpDBProps().getString("db.cacheSize")));
				...
				for (String tableName : tableSet) {
					...
					if (!pernamentTables.contains(tableName)) { // Temporary
																// table
						...
						// edwww
						// for (Select selectQuery :
						// plan.getScript().getSelectQueries()) {
						// if
						// (selectQuery.getParsedSqlQuery().getResultTable().equals(tableName))
						// {
						// for (String column :
						// selectQuery.getParsedSqlQuery().getPartitionColumns())
						// {
						// resultTable.addPartitionColumn(column);
						// }
						// break;
						...
						resultTable.getTable()
								.setSqlQuery(sqlQueries.get(queryExitMessageList.get(0).outTableInfo.getTableName()));
						resultTable.getTable().setSize(totalSize);
						...
						try {
							sqlQuery = SQLQueryParser.parse(
									resultTable.getTable().getSqlQuery().replaceAll("_", " ").replaceAll("\\ {2,}",
											"_"),
									new madgik.exareme.master.queryProcessor.decomposer.dag.NodeHashValues());
