...
import madgik.exareme.master.connector.rmi.MultipleTableReaderInputStream;
...
public class HttpAsyncDecomposerHandler implements HttpAsyncRequestHandler<HttpRequest> {
	...
	public void handle(final HttpRequest httpRequest, final HttpAsyncExchange httpExchange, final HttpContext context)
		...
		new Thread() {
			@Override
			public void run() {
				DefaultNHttpServerConnection serverCon = (DefaultNHttpServerConnection) context
				...
				try {
					if (query.startsWith("addFederatedEndpoint")) {
						...
						while (first.next()) {
							...
							for (int i = 1; i < first.getMetaData().getColumnCount() + 1; i++) {
								if (i == 5) {
									// sqlite jdbc bug, starts seq from 0,
									// should be 1
						...
						try {
								...
								if ((timeoutMs > 0 && System.currentTimeMillis() - start > timeoutMs)
									...
									for (SQLQuery q : subqueries) {
										...
										try {
											...
											// log.debug("giving hash id for:
											// "+q.getResultTableName());
										...
										if (!q.isTemporary()) {
											resultTblName.put(q.getTemporaryTableName(), q.getStringOutputs());
										}
							...
							if (a) {
								...
								if (!send1 && !streamingResult) {
									log.debug("Sending Result Table : " + resultTblName);

									String firstTblName = resultTblName.keySet().iterator().next();
									httpResponse.setEntity(new InputStreamEntity(
											dbClient.readTable(firstTblName, true, resultTblName.get(firstTblName))));
